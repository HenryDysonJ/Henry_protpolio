...
import AppDownload from '@/app/assets/downloadApp'

const CardList = ({ title, link, logo, type }: { title: string, link: string, logo: string, type?: string }) => {
  return (
    <Card isFooterBlurred className="w-full h-[300px] col-span-12 sm:col-span-7 rounded-2xl">
      ...
      <CardFooter className="absolute flex flex-row justify-between items-center bg-black/40 bottom-0 z-10 border-t-1 border-default-600 dark:border-default-100 px-3 py-1">
        ...
        <a href={link} target='_blank'>
          <Button startContent={type ==='mobile' ? <AppDownload /> : <EyeIcon />} className='w-max p-0 cursor-pointer' radius="full" size="sm" />
        </a>
      </CardFooter>
    </Card>
  )
...
type PropType = {
  slides: { title: string, demoLink: string, logo: string ,type?:string}[]
          ...
          {slides.map((item, index) => (
            <div className="embla__slide" key={index}>
              <div className="embla__slide__number">
                <CardList title={item?.title} link={item?.demoLink} logo={item?.logo} type={item?.type} />
              </div>
            </div>
          ))}
        </div>
